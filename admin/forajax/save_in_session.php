...
// $packing_size = (isset($_GET['packing_size']) ? $_GET['packing_size'] : '');
...
if (isset($_SESSION['cart'])) {
	...
	$check_available = check_duplicate_product($company_name,$product_name,$unit_name,$packing_size);

	$available_qty=0;
	$check_the_qty=0;

	if ($check_available==0) {
		$available_qty = check_qty($company_name,$product_name,$unit_name,$packing_size,$link);

		if ($available_qty>=$qty) {
			$b = array("company_name"=>$company_name, "product_name"=>$product_name, "unit_name"=>$unit_name, "packing_size"=>$packing_size,"price"=>$price,"qty"=>$qty);
			...
			echo "Entered qty is not available";
              ...
              $av_qty = 0;
              $exist_qty = 0;
              $exist_qty = check_the_qty($company_name,$product_name,$unit_name,$packing_size);
              $exist_qty = $exist_qty+$qty;
              $av_qty = check_qty($company_name,$product_name,$unit_name,$packing_size,$link);
              if ($av_qty>=$exist_qty) {
              	  $check_product_no_session = check_product_no_session($company_name,$product_name,$unit_name,$packing_size);
              	  $b = array("company_name"=>$company_name, "product_name"=>$product_name, "unit_name"=>$unit_name,"packing_size"=>$packing_size,"price"=>$price,"qty"=>$exist_qty);
...
$available_qty = check_qty($company_name,$product_name,$unit_name,$packing_size,$link);
			...
			if ($available_qty>=$qty) {
                ...
                $_SESSION['cart']=array(array("company_name"=>$company_name, "product_name"=>$product_name, "unit_name"=>$unit_name,"packing_size"=>$packing_size,"price"=>$price,"qty"=>$qty));
